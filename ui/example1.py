...
class Card(QGraphicsPixmapItem):
    ...
    def load_images(self):
        ...
        self.image_back = QPixmap(
            os.path.join('images', 'dos.png'))
        self.image_back = (self.image_back).scaledToWidth(card_width)
        self.face = self.image_face
        self.back = self.image_back
...
#        if self.stack!=None :
#            self.orientation = self.stack.orientation
#            transform = QTransform()
#            transform = transform.rotate((self.orientation)*90)
#            self.face = self.image_face.transformed(transform)
#            self.back = self.image_back.transformed(transform)
...
class StackBase(QGraphicsRectItem):
    ...
    def __init__(self, number=0,*args, **kwargs):
        ...
        if (number<=3) and (number%2==1) :
            self.orientation = ((number+1)%4) - 1  #Equal to 1 +90° and -1 -90°
            self.setRect(QRectF(CARD_RECT_PAIR))
            
        else :
            self.setRect(QRectF(CARD_RECT_ODD))
            self.orientation = int(not(((number+1)%2) or (number>3))) # Equal to 0 if the stack 
                            # number is over 5 or under 4 and pair.
...
#        self.scene.addItem(name)
        ...
        self.Joueur3 = QWidget(self)
        self.Joueur3.setGeometry(210, 10, 31, 31)
        self.Joueur3.setObjectName("Joueur3")
        
        self.Joueur4 = QWidget(self)
        self.Joueur4.setGeometry(410, 160, 31, 31)
        self.Joueur4.setObjectName("Joueur4")

        self.Joueur2 = QWidget(self)
        self.Joueur2.setGeometry(20, 160, 31, 31)
        self.Joueur2.setObjectName("Joueur2")

        self.ScoreJ1 = QLCDNumber()
        self.ScoreJ1.setGeometry(WORK_STACK_X[0]+200,WORK_STACK_Y[0]+100, 100, 50)           #(CENTER_X+120, CENTER_Y+300, 100, 50)
        self.ScoreJ1.setObjectName("ScoreJ1")
        self.scene.addWidget(self.ScoreJ1)

        self.ScoreJ2 = QLCDNumber()
        self.ScoreJ2.setGeometry(CENTER_X-450, CENTER_Y+120, 100, 50)
        ...
        self.ScoreJ3.setGeometry(WORK_STACK_X[0]-200, 100, 100, 50)
        ...
        self.BetFiller = QSpinBox(self)
        self.BetFiller.setGeometry(400, 280, 42, 22)
        self.BetFiller.setObjectName("BetFiller")
        self.scene.addWidget(self.BetFiller)

        self.label_2 = QLabel(self)
        self.label_2.setGeometry(410, 250, 35, 20)
        font = QFont()
        font.setPointSize(10)
